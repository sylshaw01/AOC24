...
//
//okay I accidentally had a hint because I looked on the reddit, the hint I got was `linear algebra` and so I think the solution goes as follows
//
//  ( b1x   b2x ) ( apresses) = (tx) 
//  ( b1y   b2y ) ( bpresses) = (ty)
//			so you just need to invert the matrix and if apresses and bpresses are integers, then you 
...
// I think also this solution is unique? unless the buttons are colinear
//
...
int main(){
	...
	for(int i{0};i<t;i++){
		...
		} else if(aprep/det >100 || bprep/det>100){
		}else{
